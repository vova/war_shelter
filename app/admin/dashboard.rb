  ...
  content title: proc { I18n.t('active_admin.dashboard') } do
    columns do
      column do
        panel 'Users to pay attention' do
          paginated_collection(User.to_pay_attention(current_admin_user)
                                   .page(params[:page])
                                   .per(10), download_links: false) do
            table_for collection do |t|
              ...
              t.column('destination') { |user| user&.destination }
    ...
    columns do
      panel 'Places to pay attention' do
        paginated_collection(Place.to_pay_attention(current_admin_user)
                                  .page(params[:page])
                                  .per(5), download_links: false) do
          table_for collection do |t|
            t.column('name') { |place| link_to(place.name, admin_place_path(place)) }
            t.column('city') { |place| place&.city }
            t.column('region') { |place| place&.region }
